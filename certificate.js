...
// 将中文金额转换为数字
function convertChineseToNumber() {
    ...
    try {
        ...
        amount = convertChineseToNumberCore(chineseText)
    } catch (e) {
...
// 解析中文数字
function convertChineseToNumberCore(chinese) {
    // 去除空格和“整”字（如果有）
...
// 填充金额到表格
function fillAmountBoxes(amount, currencySymbol = '¥') {
    // 转换为字符串，保留两位小数
...
// 更新复选框组的状态
function updateGroupCheckboxes(group) {
...
// 更新单个复选框项的状态
function updateCheckboxItemState(checkbox) {
...
function generateWarrantNumber() {
    ...
    const counter = String(warrantCounter).padStart(4, '0');

    warrantCounter++;

    return `${year}${month}${day}${hour}${minute}${second}${counter}`;
}
...
// 计算两年后的日期
function getTwoYearsLater(dateStr) {
...
async function handleFileImport(event) {
    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            const result = await fillData(data[i]);
            await new Promise(resolve => setTimeout(resolve, 1000)); // 等待页面更新

            // 使用html2canvas截图
            await captureScreenshot(result.buyer, result.amount);
...
// 使用html2canvas截图
async function captureScreenshot(buyerName, amount) {
    try {
        // 隐藏导入按钮区域
